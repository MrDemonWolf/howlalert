...
- **Native polish pass 2** (uncommitted as of 2026-05-30): four items, all build-clean (`xcodebuild -scheme HowlAlert` SUCCEEDED, 0 warnings) —
...
- [ ] Polish DONE (native pass 2, uncommitted): live relative timer ✓; native `Settings` scene ✓ (General + About). REMAINING: **Notifications settings tab + local-alert firing** (thresholds → `UNUserNotificationCenter` on entering warn/crit, gated + deduped — deliberately deferred so no dead controls ship); Stop-hook binary auto-bundle + auto-register; wolf template-image menu-bar icon (replace SF-Symbol placeholder); wire the popover tab bar to real per-tab content.
