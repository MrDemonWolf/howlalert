...
> contents of those files — your prompts, your code, your transcripts — never leave
> your device.** If you enable push on the iPhone or Apple Watch apps, the short
> usage summary shown in the notification (such as the percent of your window left
> and when it resets) is relayed through our server and Apple's push service to
> reach your devices. Our server stores only what's needed to deliver those
> notifications (a device token and a pairing code) — not your usage history. We
> don't sell your data and we don't run third-party ad or analytics trackers.
...
HowlAlert.** The one exception is push: if you turn on notifications for the iPhone
or Apple Watch apps, the brief usage summary that appears in the notification (for
example, the percent of your window left and its reset time) is sent to our relay
and Apple's APNs solely to deliver that notification to your devices. We never see
your prompts, your code, your transcripts, or Claude's responses.
...
| **Push notification content (usage summary)** | To show your remaining window on your iPhone/Watch | Relayed through our server + Apple APNs; held only transiently in cache | Held only long enough to deliver the notification, then discarded; never written to our database |
...
delete them when you unpair, uninstall, or ask us to. Cache entries — including any
usage summary in transit for a push — auto-expire under short time-to-live windows
and are never written to our database. On-device data stays on your device under
your control.
