#!/bin/bash
# create-dmg.sh — Create a styled HowlAlert DMG installer.
# Bare hdiutil + AppleScript Finder layout (no node-appdmg, no npm create-dmg).
# Mirrors mrdemonwolf/wolfwave scripts/create-dmg.sh.
...
hdiutil create -srcfolder "$BUILDS_DIR/staging" \
    -volname "HowlAlert" -fs HFS+ -format UDRW -ov \
...
osascript \
    ...
    -e '    set position of item "HowlAlert.app" of container window to {175, 190}' \
...
LEFTOVER=$(hdiutil info 2>/dev/null | awk '/\/Volumes\/HowlAlert/ {print $1; exit}')
if [ -n "$LEFTOVER" ]; then
