...
final class PushService: ObservableObject {
	...
	func checkAndPush(
		...
		// Reset tracking when usage drops below the lowest threshold
		if usagePercent < 60 {
			lastPushedThreshold = 0
			return
		}

		// Find the highest threshold that usage has crossed
		guard let crossedThreshold = thresholds.last(where: { usagePercent >= $0 }) else {
			return
		}

		// Only push when crossing a NEW threshold
	...
	/// Build the JSON payload matching `worker/src/types.ts` `PushPayload`.
	private func buildPayload(
		...
		return [
			...
			"pacePercent": round(paceState.percentDelta * 10) / 10,
		]
	}

	/// Send HTTP POST to the Worker's `/push` endpoint.
